...
from john_data import EXERCISES

ROOT = Path(__file__).resolve().parents[1] / "john"
...
for entry in EXERCISES:
    ...
    solution_path = SOL_DIR / f"{number}-{slug}.cs"
    solution_path.write_text(entry["solution"] + "\n", encoding="utf-8")

print(f"Wrote {len(EXERCISES)} exercises for John.")
